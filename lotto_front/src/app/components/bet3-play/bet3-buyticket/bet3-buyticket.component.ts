...
@Component({
  selector: 'app-bet3-buyticket',
  templateUrl: './bet3-buyticket.component.html',
  styleUrls: ['./bet3-buyticket.component.css'],
  animations: [
    trigger('cardSpinner', [
...
export class Bet3BuyticketComponent implements OnInit {
    ...
    if(this.chosenArray.length != 0)
    {
      ...
      if(this.checkAddAvailable(obj))
      {
        if(this.checkSameVal(obj))
        {
          ...
          let cdata = {
            draw_id: this.lastDrawDatas[0].data._id,
  ...
  goToBit3Video() {
    ...
    // this.router.navigate(['/loto/bet-video/bit3']);
  }
